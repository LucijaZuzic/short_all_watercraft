    ...
    for nf2 in range(sf2):
        for dirname in os.listdir("retry/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/data_provider/"):
            ...
            for ws_use in ws_range: 
                ...
                if not os.path.isdir("results/all_array_" + dirname + "_" + str(ws_use) + "_ws2_" + str(nf1 + 1) + "_" + str(nf2 + 1) + "_test/"):
                    continue

                if os.path.isdir("UniTS_final_res_ws2/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + dirname + "/" + str(ws_use) + "/"):
                    continue

                print("results/all_array_" + dirname + "_" + str(ws_use) + "_ws2_" + str(nf1 + 1) + "_" + str(nf2 + 1) + "_test/")

                for varname in os.listdir("retry/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/dataset"):

                    ordi = setse[varname][1:]
                    ordi.append(setse[varname][0])
                    
                    if not os.path.isfile("results/all_array_" + dirname + "_" + str(ws_use) + "_ws2_" + str(nf1 + 1) + "_" + str(nf2 + 1) + "_test/preds_transformed_" + varname):
                        continue
            
                    print("results/all_array_" + dirname + "_" + str(ws_use) + "_ws2_" + str(nf1 + 1) + "_" + str(nf2 + 1) + "_test/preds_transformed_" + varname)

                    file_pd = pd.read_csv("retry/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/dataset/" + varname + "/newdata_TEST.csv", index_col= False) 

                    file_pd_transformed_pred = np.array(load_object("results/all_array_" + dirname + "_" + str(ws_use) + "_ws2_" + str(nf1 + 1) + "_" + str(nf2 + 1) + "_test/preds_transformed_" + varname))
                    file_pd_transformed_true = np.array(load_object("results/all_array_" + dirname + "_" + str(ws_use) + "_ws2_" + str(nf1 + 1) + "_" + str(nf2 + 1) + "_test/trues_transformed_" + varname))
                    file_pd_transformed_xs = np.array(load_object("results/all_array_" + dirname + "_" + str(ws_use) + "_ws2_" + str(nf1 + 1) + "_" + str(nf2 + 1) + "_test/xs_transformed_" + varname))
                    ...
                    for ix_newvar in range(len(ordi)):
                        ...
                        if not os.path.isdir("UniTS_final_res_ws2/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + dirname + "/" + str(ws_use) + "/"):
                            os.makedirs("UniTS_final_res_ws2/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + dirname + "/" + str(ws_use) + "/")

                        df_new.to_csv("UniTS_final_res_ws2/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + dirname + "/" + str(ws_use) + "/" + newvar + ".csv", index = False) 
                        # count the number of missing/NaN values in each row
                        row_nan_count = df_new.isna().sum(axis=0)

                        print(row_nan_count)
                        print("UniTS_final_res_ws2/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + dirname + "/" + str(ws_use) + "/" + newvar + ".csv", r2_score(preds_smv, actual_smv))