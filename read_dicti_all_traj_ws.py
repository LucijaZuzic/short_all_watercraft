...
dicti_all = load_object("dicti_all_traj_ws")
...
start_of_table = "\\begin{table}[!t]\n\t\\begin{center}\n\t\t\\caption{METRICNAME for the trajectories in the USENUMBER k-fold testing dataset estimated using VARNAME, the UniTS model, different forecasting times, and k-fold validation datasets.}\n\t\t\\label{tab:k-fold_USENUMBER_test_VARNAME_METRICNAME}\n\t\t\\resizebox{\linewidth}{!}{"
    ...
    for varname in vartouse:
        for nf1 in range(sf1):
            ...
            if "R2" in metric_name_use:
                ...
                for val_ws in list_ws:
                    str_pr = str_pr.replace("$" + str(min_col[val_ws]) + "$", "$\\mathbf{" + str(min_col[val_ws]) + "}$")
            newstart = start_of_table.replace("USENUMBER", translate_num[nf1 + 1]).replace("METRICNAME", metric_name_use).replace("VARNAME_", varname.replace(" ", "_") + "_").replace("NRMSE ", "NRMSE (\%) ").replace("R2 ", "$R^{2}$ (\%) ").replace("VARNAME", translate_varname[varname])
...
start_of_table = "\\begin{table}[!t]\n\t\\begin{center}\n\t\t\\caption{METRICNAME for the trajectories in the k-fold testing datasets estimated using VARNAME, the UniTS model, different forecasting times, and k-fold validation datasets.}\n\t\t\\label{tab:k-fold_test_VARNAME_METRICNAME}\n\t\t\\resizebox{\linewidth}{!}{"
end_of_table = "\t\t\\end{tabular}}\n\t\\end{center}\n\\end{table}\n"
for metric_name_use in metrictouse:
    for varname in vartouse:
        duplicate_val_all = True
...
start_of_table = "\\begin{table}[!t]\n\t\\begin{center}\n\t\t\\caption{The average METRICNAME across k-fold validation datasets, with standard deviation in brackets, for the trajectories in the k-fold testing datasets estimated using VARNAME, the UniTS model, and different forecasting times.}\n\t\t\\label{tab:avg_k-fold_test_VARNAME_METRICNAME}\n\t\t\\resizebox{\linewidth}{!}{"
end_of_table = "\t\t\\end{tabular}}\n\t\\end{center}\n\\end{table}\n"
for metric_name_use in metrictouse:
    for varname in vartouse:
        duplicate_val_all = True
...
start_of_table = "\\begin{table}[!t]\n\t\\begin{center}\n\t\t\\caption{The average METRICNAME across k-fold validation datasets, with standard deviation in brackets, for the trajectories in the k-fold testing datasets estimated by the UniTS model using different trajectory estimation methods, and forecasting times.}\n\t\t\\label{tab:avg_k-fold_test_all_traj_METRICNAME}\n\t\t\\resizebox{\linewidth}{!}{"
end_of_table = "\t\t\\end{tabular}}\n\t\\end{center}\n\\end{table}\n"
for metric_name_use in metrictouse:
