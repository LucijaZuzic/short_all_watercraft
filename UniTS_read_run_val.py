...
for dirname in os.listdir("retry/data_provider/"):
    ...
    for ws_use in ws_range: 
        ...
        if not os.path.isdir("results/all_array_" + dirname + "_" + str(ws_use) + "_val/"):
            continue

        print("results/all_array_" + dirname + "_" + str(ws_use) + "_val/")
        ...
        for varname in os.listdir("retry/dataset"):
            ...
            if not os.path.isfile("results/all_array_" + dirname + "_" + str(ws_use) + "_val/preds_transformed_" + varname):
                continue
    
            print("results/all_array_" + dirname + "_" + str(ws_use) + "_val/preds_transformed_" + varname)

            file_pd = pd.read_csv("retry/dataset/" + varname + "/newdata_VAL.csv", index_col= False) 

            file_pd_transformed_pred = np.array(load_object("results/all_array_" + dirname + "_" + str(ws_use) + "_val/preds_transformed_" + varname))
            file_pd_transformed_true = np.array(load_object("results/all_array_" + dirname + "_" + str(ws_use) + "_val/trues_transformed_" + varname))
            file_pd_transformed_xs = np.array(load_object("results/all_array_" + dirname + "_" + str(ws_use) + "_val/xs_transformed_" + varname))
            ...
            for ix_newvar in range(len(ordi)):
                ...
                if not os.path.isdir("UniTS_final_res_val/" + dirname + "/" + str(ws_use) + "/"):
                    os.makedirs("UniTS_final_res_val/" + dirname + "/" + str(ws_use) + "/")

                df_new.to_csv("UniTS_final_res_val/" + dirname + "/" + str(ws_use) + "/" + newvar + ".csv", index = False) 

                print("UniTS_final_res_val/" + dirname + "/" + str(ws_use) + "/" + newvar + ".csv", r2_score(preds_smv, actual_smv))