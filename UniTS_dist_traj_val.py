...
for dn in os.listdir("UniTS_final_result_val/"):

    if "BLEU" in dn:
        continue

    if not os.path.isdir("mosaic_UniTS_val/" + dn):
        os.makedirs("mosaic_UniTS_val/" + dn)

    if not os.path.isdir("mosaic_UniTS_all_val/" + dn):
        os.makedirs("mosaic_UniTS_all_val/" + dn)
...
for dn in os.listdir("UniTS_final_result_val/"):
    ...
    if os.path.isfile("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/distance_predicted_new"):
        distance_predicted_new = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/distance_predicted_new")
    ...
    for metric in metric_names:
        ...
        predicted_all = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/predicted_all")
        y_test_all = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/y_test_all")
        ws_all = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/ws_all")

        actual_long = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/actual_long")
        actual_lat = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/actual_lat")
        predicted_long = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/predicted_long")
        predicted_lat = load_object("UniTS_final_result_val/" + model_name.replace("UniTS_", "") + "/predicted_lat")
        ...
        for ws_use in predicted_long[model_name]:
            ...
            for dist_name in predicted_long[model_name][ws_use]:
                ...
                for v in set(int_veh):
                    ...
                    for k in predicted_long[model_name][ws_use][dist_name]:
                        ...
                        if use_draw:
                            ...
                            filename = "mosaic_UniTS_all_val/" + dn + "/Vehicle_" + vehicle + "_events_" + ride + "_" + model_name + "_" + str(ws_use) + "_" + dist_name + "_" + dist_name.replace("long", "lat") + "_test_mosaic.png"
                    ...
                    if use_draw:
                        ...
                        filename_veh = "mosaic_UniTS_val/" + dn + "/Vehicle_" + str(v) + "_" + model_name + "_" + str(ws_use) + "_" + dist_name.replace("long", "lat") + "_test_mosaic.png"
                        draw_mosaic(all_actual_vehicle, all_predicted, filename_veh)

                if use_draw:

                    filename = "mosaic_UniTS_val/" + dn + "/all_" + model_name + "_" + str(ws_use) + "_" + dist_name.replace("long", "lat") + "_test_mosaic.png"
                ...
                if model_name not in dicti_to_print[dist_name]:
                    dicti_to_print[dist_name][model_name] = dict()
                    
                if str(ws_use) not in dicti_to_print[dist_name][model_name]:
                    dicti_to_print[dist_name][model_name][str(ws_use)] = dict()

                dicti_to_print[dist_name][model_name][str(ws_use)]["R2_wt"] = r2_pred_wt
                dicti_to_print[dist_name][model_name][str(ws_use)]["MAE_wt"] = mae_pred_wt
                dicti_to_print[dist_name][model_name][str(ws_use)]["RMSE_wt"] = rmse_pred_wt
                dicti_to_print[dist_name][model_name][str(ws_use)]["Euclid"] = np.average(vals_avg)
                dicti_to_print[dist_name][model_name][str(ws_use)]["R2"] = r2_pred
                dicti_to_print[dist_name][model_name][str(ws_use)]["MAE"] = mae_pred
                dicti_to_print[dist_name][model_name][str(ws_use)]["RMSE"] = rmse_pred

    save_object("UniTS_final_result_val/" + dn + "/distance_predicted_new", distance_predicted_new)
...
list_ws = sorted([int(x) for x in dicti_to_print["long no abs"][model_name]])

dicti_all_traj_val = dict()
if os.path.isfile("dicti_all_traj_val"):
    dicti_all_traj_val = load_object("dicti_all_traj_val")
    ...
    for varname in dicti_to_print:
        ...
        for model_name_use in dicti_to_print[varname]:
            ...
            for val_ws in list_ws: 
                ...
                if varname not in dicti_all_traj_val:
                    dicti_all_traj_val[varname] = dict()
                if model_name_use not in dicti_all_traj_val[varname]:
                    dicti_all_traj_val[varname][model_name_use] = dict()
                if str(val_ws) not in dicti_all_traj_val[varname][model_name_use]:
                    dicti_all_traj_val[varname][model_name_use][str(val_ws)] = dict()
                if metric_name_use not in dicti_all_traj_val[varname][model_name_use][str(val_ws)]:
                    dicti_all_traj_val[varname][model_name_use][str(val_ws)][metric_name_use] = dict()
                dicti_all_traj_val[varname][model_name_use][str(val_ws)][metric_name_use] = vv
...
save_object("dicti_all_traj_val", dicti_all_traj_val)