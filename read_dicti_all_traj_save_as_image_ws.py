...
end_of_table = "\n\t\\caption{METRICNAME for the trajectories in the k-fold testing datasets estimated using VARNAME, different RNN models, forecasting times, and k-fold validation datasets.}\n\t\\label{fig:k-fold_test_VARNAME_METRICNAME}\n\\end{figure}\n"
    ...
    for varname in vartouse:
        ...
        while too_small or duplicate_val_all:
            ...
            if rv_metric > 3 or mul_metric > 6:
                break
        if not os.path.isdir("new_img_traj_ws"):
            os.makedirs("new_img_traj_ws")
        ...
        for nf1 in range(sf1):
            ...
            if nf1 == sf1 - 1:
                ...
                plt.legend(ncol = 2)
        plt.savefig("new_img_traj_ws/" + metric_name_use + "_" + varname + ".png", bbox_inches = "tight")
        plt.close()
        ...
        newstart = start_of_table.replace("FILENAME", "new_img_traj_ws/" + metric_name_use + "_" + varname + ".png")
