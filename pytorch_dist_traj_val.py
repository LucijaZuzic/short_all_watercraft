...
for mod_use in modes:
    ...
    if os.path.isfile("pytorch_result_val/" + mod_use + "/distance_predicted_new"):
        distance_predicted_new = load_object("pytorch_result_val/" + mod_use + "/distance_predicted_new")

    metric_names = ["euclidean"] 

    if not os.path.isdir("mosaic_pytorch_val/" + mod_use):
        os.makedirs("mosaic_pytorch_val/" + mod_use)

    if not os.path.isdir("mosaic_pytorch_val_all/" + mod_use):
        os.makedirs("mosaic_pytorch_val_all/" + mod_use)
            ...
            for ws_use in predicted_long[model_name]:
                ...
                for hidden_use in predicted_long[model_name][ws_use]:
                    ...
                    for dist_name in predicted_long[model_name][ws_use][hidden_use]:
                        ...
                        for v in set(int_veh):
                            ...
                            for k in predicted_long[model_name][ws_use][hidden_use][dist_name]:
                                ...
                                if use_draw:
                                    ...
                                    filename = "mosaic_pytorch_val_all/" + mod_use + "/Vehicle_" + vehicle + "_events_" + ride + "_" + model_name + "_" + str(ws_use) + "_" + str(hidden_use) + "_" + dist_name + "_" + dist_name.replace("long", "lat") + "_test_mosaic.png"
                            ...
                            if use_draw:
                                ...
                                filename_veh = "mosaic_pytorch_val/" + mod_use + "/Vehicle_" + str(v) + "_" + model_name + "_" + str(ws_use) + "_" + str(hidden_use) + "_" + dist_name.replace("long", "lat") + "_test_mosaic.png"
                                draw_mosaic(all_actual_vehicle, all_predicted, filename_veh)

                        if use_draw:

                            filename = "mosaic_pytorch_val/" + mod_use + "/all_" + model_name + "_" + str(ws_use) + "_" + str(hidden_use) + "_" + dist_name.replace("long", "lat") + "_test_mosaic.png"
                            draw_mosaic(all_actual, all_predicted, filename)

                        print(model_name + "_" + str(ws_use) + "_" + str(hidden_use) + "_" + dist_name, np.round(np.average(vals_avg), 6))
                        ...
                        if model_name + "_"  + str(hidden_use) not in dicti_to_print[dist_name]:
                            dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)] = dict()
                            
                        if str(ws_use) not in dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)]:
                            dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)] = dict()

                        dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)]["R2_wt"] = r2_pred_wt
                        dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)]["MAE_wt"] = mae_pred_wt
                        dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)]["RMSE_wt"] = rmse_pred_wt
                        dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)]["Euclid"] = np.average(vals_avg)
                        dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)]["R2"] = r2_pred
                        dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)]["MAE"] = mae_pred
                        dicti_to_print[dist_name][model_name + "_"  + str(hidden_use)][str(ws_use)]["RMSE"] = rmse_pred 

    save_object("pytorch_result_val/" + mod_use + "/distance_predicted_new", distance_predicted_new)
...
list_ws = sorted([int(x) for x in dicti_to_print["long no abs"]["LSTM_Reference_256"]])
